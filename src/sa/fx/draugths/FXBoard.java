...
import sa.fx.draugths.sprite.SkyAlienPiece;
import sa.fx.draugths.sprite.SkySoldierPiece;
...
//			});
	...
	public SpritePiece buildPedinaLevelX(int color, Piece pedinassociated) {
		if (color != pedinassociated.getColor())
			throw new RuntimeException("Disegual color");
		...
		if (Piece.BLACK == color) {
			...
			if (pedinassociated.getType() == Piece.DRAUGTH)
				pedina.loadDraugthFrame();
